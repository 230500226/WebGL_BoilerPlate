...
// Step 3: Create a buffer for the vertices
    ...
    const pointVerticesCpuBuffer = new Float32Array(pointVertices);
    
    // Create a buffer on the GPU
    const pointGeoBuffer = gl.createBuffer();
    
    // Bind the buffer as the current array buffer
    gl.bindBuffer(gl.ARRAY_BUFFER, pointGeoBuffer);
    
    // Copy the vertices from the CPU buffer to the GPU buffer
    gl.bufferData(gl.ARRAY_BUFFER, pointVerticesCpuBuffer, gl.STATIC_DRAW);
...
// Step 7: Get the attributes from the shader program
    ...
    const vertexPositionAttributLocation = gl.getAttribLocation(pointShaderProgram, 'vertexPosition');
...
// Step 8: Drawing stages
    ...
    // Step 8.3: Draw the point 
        // Draw the point using the gpu program selected above finally
